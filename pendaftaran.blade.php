...
<html lang="id">
  <head>
    ...
    <style>
      ...
      .user-info { display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:6px; background:rgba(255,255,255,.12); margin-bottom:8px; }
      .user-avatar { width:36px; height:36px; border-radius:6px; background:rgba(255,255,255,.25); display:flex; align-items:center; justify-content:center; font-size:16px; }
      .user-name { font-size:12px; font-weight:700; color:#fff; }
      .user-role { font-size:10px; color:rgba(255,255,255,.6); }
      .logout-btn { width:100%; padding:10px; border:none; border-radius:6px; background:rgba(239,68,68,.15); color:#fca5a5; font-family:"Nunito",sans-serif; font-weight:700; font-size:13px; cursor:pointer; transition:.2s; }
      ...
      .stat-icon.daftar { background:#e0f2fe; color:var(--primary); }
      .stat-icon.proses { background:#fef9c3; color:#ca8a04; }
      .stat-icon.selesai { background:#dcfce7; color:#16a34a; }
      ...
      /* TOAST */
      .toast-custom { position:fixed; top:24px; right:24px; padding:14px 22px; border-radius:8px; font-size:14px; font-weight:700; color:#fff; z-index:9999; opacity:0; transform:translateY(-10px); transition:all .3s ease; pointer-events:none; }
  ...
  <body>
    ...
    <div class="sidebar">
      ...
      <nav class="sidebar-nav">
        <a class="nav-item active" onclick="showSection('input')" id="nav-input">
          <i class="bi bi-clipboard2-pulse"></i> Input Data Pasien
        </a>
        <a class="nav-item" onclick="showSection('daftar')" id="nav-daftar">
          <i class="bi bi-folder2"></i> Daftar Pasien Berobat
        </a>
      </nav>
    ...
    <div class="main">
      <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
          <div class="fw-bold fs-5" id="sectionTitle"><i class="bi bi-clipboard2-pulse"></i> Input Data Pasien</div>
      ...
      <div class="row g-3 mb-4">
        <div class="col-md-4">
          <div class="card border-0 shadow-sm p-3 d-flex flex-row align-items-center gap-3">
            <div class="stat-icon daftar"><i class="bi bi-people-fill"></i></div>
            <div>
        ...
        <div class="col-md-4">
          <div class="card border-0 shadow-sm p-3 d-flex flex-row align-items-center gap-3">
            <div class="stat-icon proses"><i class="bi bi-hourglass-split"></i></div>
            <div>
        ...
        <div class="col-md-4">
          <div class="card border-0 shadow-sm p-3 d-flex flex-row align-items-center gap-3">
            <div class="stat-icon selesai"><i class="bi bi-check-circle-fill"></i></div>
            <div>
      ...
      <div id="sectionInput">
        <div class="card border-0 shadow-sm p-4 mb-4">
          <h6 class="fw-bold mb-4"><i class="bi bi-clipboard-check"></i> Form Pendaftaran Pasien</h6>
          <div class="row g-3">
            <div class="col-md-6">
          ...
          <div class="d-flex gap-2 mt-4">
            <button class="btn btn-primary fw-bold" onclick="simpanData()"><i class="bi bi-floppy"></i> Daftarkan Pasien</button>
      ...
      <div id="sectionDaftar" style="display:none;">
        <div class="card border-0 shadow-sm p-4 mb-4">
          <h6 class="fw-bold mb-3"><i class="bi bi-folder-check"></i> Daftar Pasien Berobat Hari Ini</h6>
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0">
          <div class="modal-header border-0 pb-0">
            <h6 class="modal-title fw-bold"><i class="bi bi-pencil-square"></i> Edit Data Pasien Berobat</h6>
          ...
          <tr>
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                ${!p.sudahInput ? `<button class="btn-del-sm" onclick="hapus('${p.id}')"><i class="bi bi-trash"></i> Hapus</button>` : ""}
              </div>
            </td>
          </tr>
        `).join("");
  ...
  document.getElementById("sectionInput").style.display =
    section === "input" ? "block" : "none";

  document.getElementById("sectionDaftar").style.display =
    section === "daftar" ? "block" : "none";

  document.getElementById("sectionTitle").innerHTML =
    section === "input"
      ? '<i class="bi bi-clipboard2-pulse"></i> Input Data Pasien'
      : '<i class="bi bi-folder-check"></i> Daftar Pasien Berobat';
  ...
  if (section === "input") {
    ...
    navDaftar.classList.add("active");
    renderTable();
  }
}
